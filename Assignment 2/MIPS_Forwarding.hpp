...
#ifndef __MIPS_FORWARDING_HPP__
#define __MIPS_FORWARDING_HPP__
	...
	*/
	...
	std::tuple<int, int, std::string> locateAddress (std::string location){
		if(location.back() == ')'){
			try{
				...
				return std::make_tuple(address, offset, reg);
			}
	...
	void executeCommandsForwarded()
	{
		...
		while (!(PCcurr >= commands.size() && IfId->RegPC == -1 && IdEx->RegPC == -1 && ExM->RegPC == -1 && MW->RegPC == -1)){
            ...
            if(MW->WBcontrol[1] && (registerMap[MW->regDest] != 0) && (registerMap[MW->regDest] == registerMap[IdEx->rs]) && !(IdEx->Mcontrol[0])){
                IdEx->data1 = registers[registerMap[MW->regDest]];
            }
            if(MW->WBcontrol[1] && (registerMap[MW->regDest] != 0) && (registerMap[MW->regDest] == registerMap[IdEx->rt])){
                IdEx->data1 = registers[registerMap[MW->regDest]];
            }
            if(ExM->WBcontrol[1] && (registerMap[ExM->regDest] != 0) && (registerMap[ExM->regDest] == registerMap[IdEx->rs]) && !(IdEx->Mcontrol[0])){
                IdEx->data1 = ExM->result;
            }
            if(ExM->WBcontrol[1] && (registerMap[ExM->regDest] != 0) && (registerMap[ExM->regDest] == registerMap[IdEx->rt])){
                IdEx->data2 = ExM->result;
            }
			...
			if(clockCycles>=4 && (PCcurr < (commands.size()+3)) && ExM->RegPC != -1){
				if(ExM->Mcontrol[0]){
                    ...
                    if(MW->WBcontrol[1] && (registerMap[MW->regDest] == registerMap[ExM->rs])){
                        ExM->writeData = MW->result;
                    }
					data[(ExM->result)/4] = ExM->writeData;
					MW->memData = 0;
					memChange = std::to_string((ExM->result)/4) + " " + std::to_string(ExM->writeData);
				}
					...
					MW->memData = data[(ExM->result)/4];
				}
				MW->WBcontrol = ExM->WBcontrol; MW->result = ExM->result; MW->regDest = ExM->regDest; MW->rs = ExM->rs;
			}
			MW->RegPC = ExM->RegPC;
            ...
            if(clockCycles>=3 && (PCcurr < (commands.size()+2)) && IdEx->RegPC != -1){
				...
				ExM->regDest = (IdEx->EXcontrol[1])? IdEx->rt : IdEx->rd; ExM->writeData = IdEx->data2; ExM->rs = IdEx->rs;
				if(ExM->Mcontrol[2]){
					if(ExM->result == 1){
            ...
            if(clockCycles>=2 && (PCcurr < (commands.size()+1)) && IfId->RegPC != -1){
                ...
                if(ExM->Mcontrol[1] && (((registerMap[ExM->regDest] == registerMap[IdEx->rs]) && (IfId->instr[0] != "sw")) || (registerMap[ExM->regDest] == registerMap[IdEx->rt]))){
                    IdEx->RegPC = -1; PCcurr = IfId->RegPC; 
                }
            }
