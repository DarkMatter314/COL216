...
#ifndef __MIPS_79FORWARDING_HPP__
#define __MIPS_79FORWARDING_HPP__
...
struct MIPS_Architecture
{
	...
	*/
    ...
    void clear(RRExReg *RREx){
        RREx->EXcontrol = {0,0}; RREx->Mcontrol = {0,0,0}; RREx->WBcontrol = {0,0}; RREx->ALUOp = 0; RREx->type = 0;
        RREx->rs = RREx->rt = RREx->rd = ""; RREx->data1 = RREx->data2 = RREx->dataIm = 0; RREx->branch = "";
    }
    ...
    void clear(ExDM1Reg *ExDM1){
        ExDM1->Mcontrol = {0,0,0}; ExDM1->WBcontrol = {0,0}; ExDM1->regDest = ""; 
        ExDM1->writeData = 0; ExDM1->result = 0;
    }
	...
	void clear(DM1DM2Reg *DM1DM2){
		DM1DM2->Mcontrol = {0,0,0}; DM1DM2->WBcontrol = {0,0}; DM1DM2->regDest = ""; 
		DM1DM2->result = 0; DM1DM2->memData = 0; DM1DM2->writeData = 0;
	}
	...
	void ControlUnit  (std::vector<std::string> instr, Id1Id2Reg* Id1Id2){
		...
		if(instr[0] ==  "add"){
			...
			std::tie(Id1Id2->dataIm, Id1Id2->rs) = locateAddress(instr[2]); Id1Id2->rd = instr[1];
		}
		else if(instr[0] == "sw"){
	...
	void executeCommands79Forwarded()
	{
		...
		while (!(PCcurr >= commands.size() && If1If2->RegPC == -1 && If2Id1->RegPC == -1 && Id1Id2->RegPC == -1 && Id2RR->RegPC == -1 && RREx->RegPC == -1 && ExDM1->RegPC == -1 && DM1DM2->RegPC == -1 && DM2RW->RegPC == -1)){
            ...
            // Forwarding logic
            if(DM2RW->RegPC != -1 && DM2RW->WBcontrol[1] && (registerMap[DM2RW->regDest] == registerMap[RREx->rs]) && !(RREx->Mcontrol[0]))  RREx->data1 = DM2RW->result;
            if(DM2RW->RegPC != -1 && DM2RW->WBcontrol[1] && (registerMap[DM2RW->regDest] == registerMap[RREx->rt]))  RREx->data2 = DM2RW->result;
            if(ExDM1->RegPC != -1 && ExDM1->Mcontrol[0] && (registerMap[DM2RW->regDest] == registerMap[RREx->rs])){
                if(DM2RW->WBcontrol[0]) ExDM1->writeData = DM2RW->memData;
                else ExDM1->writeData = DM2RW->result;
            }
           ...
           // RW stage
            ...
            if(clockCycles>=8 && DM1DM2->RegPC != -1){
                ...
                if(DM1DM2->Mcontrol[0]){
					...
					data[DM1DM2->result] = DM1DM2->writeData;
					DM2RW->memData = 0;
			...
			if(clockCycles>=7 && ExDM1->RegPC != -1){
				...
				if(ExDM1->Mcontrol[1]){
					...
					DM1DM2->memData = data[ExDM1->result];
				}
            ...
            if(clockCycles>=6 && RREx->RegPC != -1){
                ...
                // Stall if sw after lw
                if(ExDM1->RegPC != -1 && ExDM1->WBcontrol[1] && RREx->WBcontrol[0] && registerMap[ExDM1->regDest] == registerMap[RREx->rs]){
                    clear(ExDM1); ExDM1->RegPC = -1; 
					printRegisters(clockCycles);
					if(memChange != ""){
						std::cout<<"1 "<<memChange<<std::endl;
						memChange = "";
				...
				clear(ExDM1); clear(DM1DM2); ExDM1->RegPC = -1;
			}
            ...
            if(clockCycles >= 5 && Id2RR->RegPC != -1){
                ...
                //stall for lw in Ex
                if(ExDM1->WBcontrol[1] && ((registerMap[ExDM1->regDest] == registerMap[Id2RR->rs]) || (registerMap[ExDM1->regDest] == registerMap[Id2RR->rt]))){
                    clear(ExDM1); clear(RREx); RREx->RegPC = -1;
					printRegisters(clockCycles);
					if(memChange != ""){
						std::cout<<"1 "<<memChange<<std::endl;
						memChange = "";
                ...
                // stall if lw in DM1
                if(DM1DM2->WBcontrol[1] && ((registerMap[DM1DM2->regDest] == registerMap[Id2RR->rs]) || (registerMap[DM1DM2->regDest] == registerMap[Id2RR->rt]))){
                    clear(RREx); RREx->RegPC = -1;
					printRegisters(clockCycles);
					if(memChange != ""){
						std::cout<<"1 "<<memChange<<std::endl;
						memChange = "";
            ...
            if(clockCycles >= 3 && If2Id1->RegPC != -1){
                ...
                if(Id2RR->RegPC != -1 && Id1Id2->type == 0 && Id2RR->type == 1){
                    ...
                    Id1Id2->RegPC = If2Id1->RegPC = If1If2->RegPC = -1; Id1Id2->type = 0;
                }
